...
-}
data Vector = V { vx :: Double, vy :: Double, vz :: Double} deriving (Show, Eq)

lengthV :: Vector -> Double
lengthV (V vx vy vz) = sqrt $ vx * vx + vy * vy + vz * vz

normalizeV :: Vector -> Vector
normalizeV v@(V vx vy vz) = V new_vx new_vy new_vz
  where
    new_vx = vx / lengthV v
    new_vy = vy / lengthV v
    new_vz = vz / lengthV v
...
dotV v1@(V vx1 vy1 vz1) v2@(V vx2 vy2 vz2) = vx1 * vx2 + vy1 * vy2 + vz1 * vz2

crossV :: Vector -> Vector -> Vector
crossV v1@(V a1 a2 a3) v2@(V b1 b2 b3) = V new_vx new_vy new_vz
  where
    new_vx = a2 * b3 - a3 * b2
    new_vy = a3 * b1 - a1 * b3
    new_vz = a1 * b2 - a2 * b1
...
addV v1@(V a1 a2 a3) v2@(V b1 b2 b3) = V new_vx new_vy new_vz
  where
    new_vx = a1 + b1
    new_vy = a2 + b2
    new_vz = a3 + b3
...
subV v1@(V a1 a2 a3) v2@(V b1 b2 b3) = V new_vx new_vy new_vz
  where
    new_vx = a1 - b1
    new_vy = a2 - b2
    new_vz = a3 - b3
