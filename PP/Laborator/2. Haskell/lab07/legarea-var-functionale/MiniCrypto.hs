...
arithmetic initial r = iterate (\x -> x + r) initial
geometric :: Num a => a -> a -> [a]
geometric initial q = iterate (\x -> x * q) initial
{-
  3. (2p)
...
randoms :: Int -> [Word8]
randoms seed = randoms2 seed (mkStdGen seed)

randoms2 :: Int -> StdGen -> [Word8]
randoms2 seed generator = [element_to_insert] ++ (randoms2 42 new_generator)
  where
    elem = fst $ next generator
    element_to_insert = fromIntegral (elem::Int) :: Word8
    new_generator = snd $ next generator
...
wordToAlpha idx = chr code
  where
    int_idx = fromIntegral (idx::Word8)::Int 
    ordin = int_idx `mod` 26
    code = ordin + 97
{-
    5. (1p)
...
tableToFunc lst character = snd pair
  where
    ordin = ord character
    idx = ordin `mod` 97
    pair = (!!) lst idx
...
createSecondList :: Int -> [Char]
createSecondList offset = [initial_character .. 'z'] ++ ['a' .. final_character]
  where
    initial_character = chr $ ord 'a' + offset
    final_character = chr $ ord 'a' + (offset - 1)

genRotTable :: Int -> [(Char, Char)]
genRotTable = zip ['a' .. 'z'] . createSecondList
...
getCharacter pair = (!!) dictionary ord_x
  where
    x = fst pair
    y = snd pair
    ord_x = ord x `mod` 97
    ord_y = ord y `mod` 97
    dictionary = createSecondList ord_y

getResult :: [(Char, Char)] -> String
getResult [] = []
getResult lst = [getCharacter (head lst)] ++ (getResult (tail lst))

encryptVigenere :: String -> String -> String
encryptVigenere = (getResult .) . myfun

myfun str key
  | length(key) < length(str) = zip str (myCycle key)
  | otherwise = zip (myCycle str) key
...
xorStringsHelper :: [Char] -> [Char] -> [(Char, Char)]
xorStringsHelper str1 str2
  | length(str1) < length(str2) = zip (myCycle str1) str2
  | length(str1) > length(str2) = zip str1 (myCycle str2)
  | otherwise = zip str1 str2

xorStringsGetResult :: [(Char, Char)] -> [Char]
xorStringsGetResult [] = []
xorStringsGetResult lst = [element_to_insert] ++ (xorStringsGetResult (tail lst))
  where
    elem = head lst
    x = fst elem
    y = snd elem
    ord_x = ord x
    ord_y = ord y
    xor_result = xor ord_x ord_y
    element_to_insert = chr xor_result

xorStrings :: [Char] -> [Char] -> [Char]                                        
xorStrings = (xorStringsGetResult .) . xorStringsHelper
