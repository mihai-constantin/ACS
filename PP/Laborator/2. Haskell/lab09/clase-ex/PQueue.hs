...
class (Ord a) => PQueue pq a where
    ...
    toList :: pq a -> [(Prio, a)]
    toList priority_queue
      | isEmpty priority_queue = []
      | otherwise = x : (toList (pop priority_queue))
        where Just x = top priority_queue

    size :: pq a -> Int
    size priority_queue
      | isEmpty priority_queue = 0
      | otherwise = 1 + (size (pop priority_queue))
...
instance (Ord a) => PQueue LeftistPQ a where
    ...
    insert elem priority_queue = (merge (Node 0 elem empty empty) priority_queue)

    top (Empty _) = Nothing
    top (Node rank root left right) = Just root

    pop priority_queue = merge (left priority_queue) (right priority_queue)
