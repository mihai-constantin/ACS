...
(define (num->base n b)
  (if (= n 0)
      '()
      (append (num->base (quotient n b) b) (cons (modulo n b) '()))))
...
(define (all-palindromes? n Bases)
  (if (null? Bases)
      #t
      (if (eq? (palindrome? (num->base n (car Bases))) #t)
...
(define (palindromes-to-n n Bases)
  (if (= n 0)
      '(0)
      (if (eq? (all-palindromes? n Bases) #f)
...
; am definit lista cu bazele prin care imi trec numarul
(define my_list '(2 3 4 5 6 7 8 9 10))

; functie care returneaza cate reprezentari in bazele din my_list sunt palindrome pentru un numer n
(define (first-b-pal2 n count my_list)
  (if (null? my_list)
      count
      (if (eq? (palindrome? (num->base n (car my_list))) #t)
          (first-b-pal2 n (+ count 1) (cdr my_list))
          (first-b-pal2 n count (cdr my_list)))))

(define (first-b-pal start b)
  (if (>= (first-b-pal2 start 0 my_list) b)
      start
...
(define (take L n)
  (if (or (zero? n) (null? L))
      '()
      (append (cons (car L) '()) (take (cdr L) (- n 1)))))

(define (substring L n)
  (if (< (length L) n)
      '()
...
(define (solve7 n digits)
  (if (= digits 0)
      '()
      (if (= (length (substring (num->base n 10) digits)) digits)
          (list->num (substring (num->base n 10) digits))
          (solve7 n (- digits 1)))))
         
(define (longest-palindrome n)
  (solve7 n (length (num->base n 10))))
