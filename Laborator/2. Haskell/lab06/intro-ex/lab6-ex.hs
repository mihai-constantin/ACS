...
-}
nr_div :: Int -> Int
nr_div n = length divisors
  where
    divisors = [x | x <- [1 .. n], n `mod` x == 0]

primes :: Int -> [Int]
primes n = [x | x <- [2, 3 .. n - 1], prim x == 1]
  where
    prim x
      | x < 2 = 0
      | otherwise  = if nr_div x == 2 then 1 else 0
...
fgroup2 x [] = [[x]]
fgroup2 x ((x1 : xs2) : xs3)
  | x == x1 = (x:x1:xs2) : xs3
  | otherwise = [x] : (x1:xs2) : xs3

group2 :: Eq a => [a] -> [[a]]
group2 = foldr fgroup2 []
...
-}
nrOcc :: Ord a => [a] -> [(a, Int)]
nrOcc lst = map (\x -> (head x, length(x))) mylist
  where
    mylist = group2 (sort lst)
...
-}
dup :: String -> String
dup string = unwords (concat ( map (\x -> [x]++[x]) lst))
  where
    lst = words string
...
isIsogram mystring = length(nrOcc lst) == length(lst)
  where
    lst = filter isLetter mystring
