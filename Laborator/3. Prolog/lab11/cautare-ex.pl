...
safe(X/Y/S, Others) :- findall(X/_/S, (member(X/_/S, Others)), Result), length(Result, 0), 
	findall(_/Y/S, (member(_/Y/S, Others)), Result2), length(Result2, 0).
...
% călători cu barca.
boat(0,1). boat(1,1). boat(2,0). boat(1,0). boat(0,2).
...
safeMisionari(Misionari, Canibali) :- 
	Misionari >= Canibali; 
	Misionari = 0, Canibali > 0.
...
parseState(stare(Mal, NM_Est, NC_Est, NM_Vest, NC_Vest), Mal, NM_Est, NC_Est, NM_Vest, NC_Vest).
...
initial_state(misionari, stare(est, 3, 3, 0, 0)).
...
final_state(misionari, stare(vest, 0, 0, 3, 3)).
...
% S1.
next_state(misionari, stare(est, NM_Est1, NC_Est1, NM_Vest1, NC_Vest1), 
					  stare(vest, NM_Est2, NC_Est2, NM_Vest2, NC_Vest2)) :- 
	boat(M, C),
	NM_Est1 >= M,
	NC_Est1 >= C,
	NM_Est2 is NM_Est1 - M,
	NM_Vest2 is NM_Vest1 + M,
	NC_Est2 is NC_Est1 - C,
	NC_Vest2 is NC_Vest1 + C,
	safeMisionari(NM_Est2, NC_Est2), 
	safeMisionari(NM_Vest2, NC_Vest2).
	
	
next_state(misionari, stare(vest, NM_Est1, NC_Est1, NM_Vest1, NC_Vest1), 
					  stare(est, NM_Est2, NC_Est2, NM_Vest2, NC_Vest2)) :- 
	boat(M, C),
	NM_Vest1 >= M,
	NC_Vest1 >= C,
	NM_Vest2 is NM_Vest1 - M,
...
preorder(N, [N|Rest]) :- 
	findall(Acc, (arc(N, List), member(Node,List), preorder(Node,Acc)), Res),
	helper(Res, Rest).
	
helper([],[]).
helper([L|List], Res):- helper(List, Acc), append(L,Acc,Res).
...
% unui arbore.
trees(Trees) :- nodes(Nodes), helperTree(Nodes,[],Trees).

helperTree([], _, []).

helperTree([Node|Rest],Visited,Tree) :-
	member(Node, Visited), 
	helperTree(Rest, Visited, Tree).

helperTree([Node|Rest],Visited,[L|Tree]) :- 
	\+member(Node, Visited), 
	preorder(Node, L), 
	append(L, Visited, Res),
	helperTree(Rest, Res, Tree).
