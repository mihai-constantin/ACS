...
myConcat([], L, L).
myConcat(L, [], L).
myConcat([Head|Tail], L2, [Head|L]) :- myConcat(Tail, L2, L).
...
myReverse([], []).
myReverse([Head|Tail], RevList):-
        myReverse(Tail, RevTail),
        myConcat(RevTail, [Head], RevList).
...
myReverseAcc([],[]).
myReverseAcc([H|T],R) :- myReverseAcc(T,RevT), myConcat(RevT,[H],R).

myReverseAcc([], L, L).
myReverseAcc([H|T], Acc, L):-
        myReverseAcc(T, [H|Acc], L).
...
extract([],_,_,[]):- !.
extract(_,0,-1,[]):- !.
extract([X|L],0,End,[X|Rez]):- End >= 0, NewEnd is End - 1, extract(L,0,NewEnd,Rez).
extract([_|L],Start,End,Rez):- Start > 0, NewStart is Start - 1, extract(L,NewStart,End,Rez).
...
factorial_helper(S,P,Rez,X):- P >= Rez, X is S, !.
factorial_helper(S,P,Rez,X):- P < Rez, S2 is S + 1, P2 = P * S2, factorial_helper(S2,P2,Rez,X).

factorial2(X,Rez):- var(Rez), factorial(X,Rez), !.
factorial2(X,Rez):- factorial_helper(1,1,Rez,X).
...
%% cazul in care e frunza
descendants(X, 0):-
        isLeaf(X).

%% cazul in care are doar un subarbore
descendants(X, N):-
        arc(X, Y),
        \+ (arc(X, Z), Z \= Y),
        descendants(Y, N1),
...
%% cazul in care are doi subarbori
descendants(X, N):-
        arc(X, Y),
        arc(X, Z),
        Y @< Z,
...
%% sameTree/2
%% verifica daca doua noduri sunt in aceeasi componenta conexa
%% sameTree(+Nod, +Nod).
...
drum(X,Y,P) :- drum_helper(X,Y,[Y],P). 

drum_helper(X,Y,L,[X|L]) :- arc(X,Y).
drum_helper(X,Y,L,P) :- arc(Z,Y), not(member(Z,L)), drum_helper(X,Z,[Z|L],P).
