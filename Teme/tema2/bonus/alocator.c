...
uint32_t ind_start, ind_start_pred, ind_start_date;
...
void ALLOC(uint32_t size) // size - NR OCTETI DIMENSIUNE DATE
{
	...
	if(ok) // AM GASIT O ZONA LIBERA DE (12 + size) OCTETI
	{
		...
		if((pos < START_BLOC) || (ind_start == START_BLOC))
		{
		...
		ind_start_date = pos - size + 1;  // POZITIA PRIMULUI OCTET DIN SECTIUNEA DE DATE A ZONEI LIBERE GASITE
...
void ALLOCALIGNED(uint32_t size, uint32_t align)
{
	...
	uint32_t i, pos, j, pos2;
	uint32_t dim = 12 + size;

	bool ok = false, gasit;

	// CAUT CEL MAI MIC MULTIPLU >= 12 PENTRU align

	uint32_t indice = align;
	...
	for(i = indice; (i + size - 1) < N; i += align)
	{
		ok = true;

		// VERIFIC DACA ZONA E LIBERA

		for(j = i - 12; j <= (i + size - 1); j++)
		{
			if(empty[j] != 0)
			{
				ok = false;
	...
	if(ok) // AM GASIT O ZONA LIBERA DE (12 + size) OCTETI
	{
		pos = i + size - 1; // POZITIA ULTIMULUI OCTET DIN ZONA LIBERA GASITA
		...
		if(gasit == true) // FAC ALLOC PE PRIMUL BLOC
		{
			//CAUT URMATORUL BLOC
		...
		else
		{
			...
			if(gasit == true)  // FAC ALLOC PE ULTIMUL BLOC
			{
				// CAUT BLOCUL PRECEDENT
			...
			else // FAC ALLOC PE UN BLOC CU 2 VECINI
			{
				// CAUT BLOCUL PRECEDENT
...
void REALLOC(uint32_t index, uint32_t size)
{
	...
	copy = (unsigned char*)calloc(N, sizeof(unsigned char));

	// FAC COPIA DATELOR DIN BLOC
	...
	if(ok_alloc == true)
	{
		...
		// Min - CATE VALORI COPIEZ IN Arena

		uint32_t k = index;

		for(i = ind_start_date; i <= ind_start_date + Min - 1; i++)
		{
			Arena[i] = copy[k];

			k++;

			if(i == (ind_start_date + Min - 1))
			{
				break;
			}
		}
