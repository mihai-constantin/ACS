...
{
	int ans =  abs(x_p - x_obs) + abs(y_p - y_obs);
	...
	{	
		...
		switch(c)
		{
			case 'w':
			{
				...
				if(NR % 2 == 0)
				{
					// VREAU SA AFLU NOILE POZITII ALE OBSTACOLELOR 
					...
					if(c2 == 'm')
					{
						dist_min = distance(x, y, x2 - 1, y2 - 1);

						X2 = x2 - 1;
						Y2 = y2 - 1;

						dist = distance(x, y, x2 - 1, y2);
					...
					if(c3 == 'm')
					{
						dist_min = distance(x, y, x3 - 1, y3 - 1);

						X3 = x3 - 1;
						Y3 = y3 - 1;

						dist = distance(x, y, x3 - 1, y3);
