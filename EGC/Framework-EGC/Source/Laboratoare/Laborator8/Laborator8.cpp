...
void Laborator8::Init()
{
	...
	spot_light_activated = true;
	cut_off_angle = 30;
	rotateDirection = glm::vec3(1);
}
...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	// turn on/off spot_light
	int spot_light_activated_pos = glGetUniformLocation(shader->program, "spot_light_activated");
	glUniform1i(spot_light_activated_pos, spot_light_activated);

	// send spotlight cuttoff angle
	int cut_off_angle_pos = glGetUniformLocation(shader->program, "cut_off_angle");
	glUniform1f(cut_off_angle_pos, cut_off_angle);
...
void Laborator8::OnKeyPress(int key, int mods)
{
	// add key press event
	if (key == GLFW_KEY_F) {
		spot_light_activated = !spot_light_activated;
	}
}
